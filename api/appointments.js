...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const result = await sqlRepository.listAppointments(session, req.query?.from, req.query?.to, req.query?.cursor, req.query?.limit);
    const sqlMs = Date.now() - sqlStartedAt;
    const response = { success: true, ...result, requestId: id };
    logRequest({ id, route: "/api/appointments", status: 200, startedAt, sqlMs, bytes: Buffer.byteLength(JSON.stringify(response)) });
    return sendJson(res, 200, response, { "Server-Timing": `sql;dur=${sqlMs}` });
  } catch (error) {
    logRequest({ id, route: "/api/appointments", status: 502, startedAt, error: error.message });
    return sendJson(res, 502, { success: false, error: "appointments_unavailable", requestId: id });
  }
};
