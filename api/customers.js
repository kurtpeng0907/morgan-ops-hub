...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const result = await sqlRepository.listCustomers(session, req.query?.cursor, req.query?.limit, req.query?.query);
    const sqlMs = Date.now() - sqlStartedAt;
    const response = { success: true, ...result, requestId: id };
    logRequest({ id, route: "/api/customers", status: 200, startedAt, sqlMs, bytes: Buffer.byteLength(JSON.stringify(response)) });
    return sendJson(res, 200, response, { "Server-Timing": `sql;dur=${sqlMs}` });
  } catch (error) {
    logRequest({ id, route: "/api/customers", status: 502, startedAt, error: error.message });
    return sendJson(res, 502, { success: false, error: "customers_unavailable", requestId: id });
  }
};
